...
type Env={DB:D1Database;ADMIN_EMAIL?:string;RESEND_API_KEY?:string;PAYMENT_FROM_EMAIL?:string;PAYPAL_CLIENT_ID?:string;PAYPAL_CLIENT_SECRET?:string;PAYPAL_ENV?:string};
...
const admin=(r:Request,e:Env)=>(r.headers.get("oai-authenticated-user-email")||"").toLowerCase()===(e.ADMIN_EMAIL||ADMIN_EMAIL).toLowerCase();
...
async function listPlans(r:Request,e:Env){
 if(!admin(r,e))return json({error:"Authorised administrator access is required."},403);
...
async function createPlan(r:Request,e:Env){
 ...
 try{await e.DB.batch([
  e.DB.prepare("INSERT INTO payment_clients(id,first_name,last_name,email,phone,company,created_at) VALUES(?,?,?,?,?,?,?)").bind(clientId,first,last,email,phone||null,company||null,now),
  e.DB.prepare("INSERT INTO payment_plans(id,client_id,reference,title,description,currency,total_amount_cents,access_token_hash,status,created_at) VALUES(?,?,?,?,?,?,?,?, 'active',?)").bind(planId,clientId,ref,title,description,currency,total,accessHash,now),
  e.DB.prepare("INSERT INTO payment_installments(id,plan_id,sequence,label,amount_cents,due_description,status,created_at) VALUES(?,?,1,?,?,?,'pending',?)").bind(depositId,planId,`Deposit · ${dp}%`,deposit,depositDue,now),
  e.DB.prepare("INSERT INTO payment_installments(id,plan_id,sequence,label,amount_cents,due_description,status,created_at) VALUES(?,?,2,?,?,?,'pending',?)").bind(finalId,planId,`Final payment · ${fp}%`,final,finalDue,now),
...
async function clientPlan(r:Request,e:Env){
 ...
 const i=await e.DB.prepare("SELECT id,sequence,label,amount_cents AS amountCents,due_description AS dueDescription,status,paid_at AS paidAt FROM payment_installments WHERE plan_id=? ORDER BY sequence").bind(p.id).all();
